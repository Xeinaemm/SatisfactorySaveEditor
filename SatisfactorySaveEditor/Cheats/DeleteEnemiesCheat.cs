...
public class DeleteEnemiesCheat : ICheat
{
    ...
    public bool AddDoggo(SaveObjectModel rootItem, SatisfactorySave saveGame)
    {
        ...
        var healthComponent = new SaveComponent("/Script/FactoryGame.FGHealthComponent", "Persistent_Level", $"Persistent_Level:PersistentLevel.Char_SpaceRabbit_C_{currentDoggoID}.HealthComponent")
        {
            DataFields = [],
            ParentEntityName = $"Persistent_Level:PersistentLevel.Char_SpaceRabbit_C_{currentDoggoID}"
        };
        var bytes = PrepareForParse("", 0);
        ...
        var doggo = new SaveEntity("/Game/FactoryGame/Character/Creature/Wildlife/SpaceRabbit/Char_SpaceRabbit.Char_SpaceRabbit_C", "Persistent_Level", $"Persistent_Level:PersistentLevel.Char_SpaceRabbit_C_{currentDoggoID}")
        {
            NeedTransform = true,
            ...
            Components =
        [
            new() {LevelName = "Persistent_Level", PathName = $"Persistent_Level:PersistentLevel.Char_SpaceRabbit_C_{currentDoggoID}.mInventory"},
            new() {LevelName = "Persistent_Level", PathName = $"Persistent_Level:PersistentLevel.Char_SpaceRabbit_C_{currentDoggoID}.HealthComponent"}
        ]
        };
        byte[] emptyDynamicStructData = [0x05, 0x00, 0x00, 0x00, 0x4e, 0x6f, 0x6e, 0x65]; // Length prefixed "None"
        using (var binaryReader = new BinaryReader(new MemoryStream(emptyDynamicStructData)))
        {
            doggo.DataFields =
            [
        ...
        FindOrCreatePath(rootItem, ["Character", "Creature", "Wildlife", "SpaceRabbit", "Char_SpaceRabbit.Char_SpaceRabbit_C"]).Items.Add(new SaveEntityModel(doggo));
    ...
    private static double Distance(Vector3 a, Vector3 b) => Math.Sqrt(((a.X - b.X) * (a.X - b.X)) + ((a.Y - b.Y) * (a.Y - b.Y)) + ((a.Z - b.Z) * (a.Z - b.Z)));
    ...
    public bool Apply(SaveObjectModel rootItem, SatisfactorySave saveGame)
    {
        ...
        var playerPosition = ((SaveEntityModel)hostPlayerModel.Items[0]).Position;

        foreach (var animalSpawner in animalSpawners.DescendantSelfViewModel)
        {
