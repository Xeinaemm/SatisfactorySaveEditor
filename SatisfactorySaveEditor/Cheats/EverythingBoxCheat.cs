...
class EverythingBoxCheat : ICheat
{
    ...
    public bool Apply(SaveObjectModel rootItem, SatisfactorySave saveGame)
    {
        ...
        var resourceStrings = new HashSet<string>();
        var radioactiveStrings = new HashSet<string>();
        ...
        foreach (var resource in resourceStrings)
        {
            if (!radioactiveStrings.Contains(resource))//because duplicates can be in the xml file in order, still need to check every string here for radioactivity
            {
                ...
                var bytes = MassDismantleCheat.PrepareForParse(resource, itemStackQuantity); //reuse mass dismantle cheat's parsing method
                using (var ms = new MemoryStream(bytes))
                using (var reader = new BinaryReader(ms))
                {
                    var prop = SerializedProperty.Parse(reader, saveGame.Header.BuildVersion);
        ...
        var skipped = "";
        foreach (var radioactiveResource in radioactiveStrings)
        {
