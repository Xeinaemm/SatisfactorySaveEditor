...
public class ResearchUnlockCheat : ICheat
{
    ...
    private static void MissingTagMsg(string tagName) => MessageBox.Show($"This save does not contain a {tagName}.\nThis means that the loaded save is probably corrupt. Aborting.", "Cannot find " + tagName, MessageBoxButton.OK, MessageBoxImage.Error);

    private static BoolPropertyViewModel CreateOrSetBoolField(SaveObjectModel model, string fieldName, bool value) => model.FindOrCreateField<BoolPropertyViewModel>(fieldName, f => f.Value = value);
}
