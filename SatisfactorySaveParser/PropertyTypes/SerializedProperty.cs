...
public abstract class SerializedProperty(string propertyName, int index)
{
    ...
    public static SerializedProperty Parse(BinaryReader reader, int buildVersion)
    {
        ...
        return size != readBytes ? throw new InvalidOperationException($"Expected {size} bytes read but got {readBytes}") : result;
    }
}