...
public class ArrayProperty(string propertyName, int index = 0) : SerializedProperty(propertyName, index)
{
    ...
    public override int SerializedLength
    {
        get
        {
            return Type switch
            {
                StructProperty.TypeName => StructProperty.GetSerializedArrayLength(Elements.Cast<StructProperty>().ToArray()),
                ObjectProperty.TypeName => 4 + Elements.Cast<ObjectProperty>().Sum(obj => obj.LevelName.GetSerializedLength() + obj.PathName.GetSerializedLength()),
                IntProperty.TypeName => (Elements.Count * 4) + 4,
                ByteProperty.TypeName => 4 + Elements.Count,
    ...
    /// </summary>
    public List<SerializedProperty> Elements { get; set; } = [];

    public override string ToString() => $"array of {Type}";
