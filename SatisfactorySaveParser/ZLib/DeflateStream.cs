...
public class DeflateStream : Stream
{
    ...
    public override int Read(byte[] buffer, int offset, int count)
    {
        ...
        return count == 0 ? 0 : Read(buffer.AsSpan(offset, count));
    }
    ...
    public unsafe int Read(Span<byte> buffer)
    {
        ...
        fixed (byte* writePtr = buffer)
        {
            ...
            while (strm.avail_out != 0)
            {
                ...
                var inCount = strm.avail_in;
                var outCount = strm.avail_out;
        ...
        fixed (byte* writePtr = workBuffer)
        {
            ...
            while (ret != ZLibReturnCode.StreamEnd)
            {
                if (strm.avail_out != 0)
                {
                    ...
                    if (ret is not ZLibReturnCode.Ok and not ZLibReturnCode.StreamEnd)
                        throw new ZLibException(ret, strm.LastErrorMessage);
                }
    ...
    {
        var bits = (int)windowBits;
        return format switch
        {
            Format.Deflate => bits * -1,
            Format.GZip => bits += 16,
            Format.ZLib => bits,
            Format.BothZLibGzip => mode == CompressionMode.Decompress
                                ? (bits += 32)
                                : throw new ArgumentException("Only support for decompressing", nameof(format)),
            _ => throw new ArgumentOutOfRangeException(nameof(format), "Not a valid format"),
        };
    }
}
